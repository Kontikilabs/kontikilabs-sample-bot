							...
							"query": {
								"bool": {
									"must": [
										...
										{ "match": { "bot_id": CONFIG.platformCredentials.botId} }
									]
								}
							}
						}
					}
					BACKEND_API.EVENTS.GetDataFromES(esData, access_token, function(data) { 	// get stories json from backend 
						if (data.items.length > 0) {
							SENDRESPOSNE.SendText(inputJSON, text, function() {
								PLATFORMTEMPLATE.CreateGenericTemplate(data.items, inputJSON, function(template){	// to create generic template of stories
									SENDRESPOSNE.SendGenericTemplate(inputJSON, template, function() {    // to send generic template of stories

									});
								})
							});
						} else {
							SENDRESPOSNE.SendText(inputJSON, "no stories available", function() {});
						}
					});
				});
		}
