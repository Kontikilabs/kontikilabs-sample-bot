			...
			LOCALSTORAGE.SaveUserConversation(localStorageData, function(localMemorycb) {
				if(userDetail.length > 0) {    	// condition if user exist in database
					...
					BACKEND_API.EVENTS.UserLogout(access_token, function(logout) {
						...
						*/
						SENDRESPONSE.SendText(inputJSON, welcomeResponseText, function() {
							SENDRESPONSE.SendTypingAction(inputJSON, 'typing_on', function(){
								setTimeout(function() {
									SENDRESPONSE.SendText(inputJSON, CONTENT.get_started.message3, function() {
										SENDRESPONSE.SendTypingAction(inputJSON.senderId, 'typing_on', function(){
											setTimeout(function() {
												SENDRESPONSE.SendText(inputJSON, CONTENT.get_started.message4, function() {
												});
